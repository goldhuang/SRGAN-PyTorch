...
def main():
	...
	for epoch in range(1, 21):
		netG.load_state_dict(torch.load('epochs/netG_epoch_'+ str(epoch*5) +'_gpu.pth'))
		...
		with torch.no_grad():
			...
			for image in dev_save_bar:
				...
				utils.save_image(image, out_path + 'epoch_%d_index_%d.png' % (epoch*5, index), padding=5)
				index += 1

			log_value('ssim', cache['ssim']/len(dev_loader), epoch*5)
			log_value('psnr', cache['psnr']/len(dev_loader), epoch*5)
