...
def main():
	...
	parser.add_argument('--num_epochs', default=30, type=int, help='training epoch')
	...
	if check_point == 0:
		for epoch in range(1, n_epoch_pretrain + 1):
			...
			if use_tensorboard:
				log_value('pretrain-generator-loss', cache['g_loss']/len(train_loader), epoch)
	...
	for epoch in range(1 + check_point, n_epoch + 1 + check_point):
		...
		cache = {'image_loss': 0, 'adversarial_loss': 0, 'g_loss': 0, 'd_loss': 0, 'ssim': 0, 'psnr': 0}
		...
		for data, target in train_bar:
			...
			# Print information by tqdm
			train_bar.set_description(desc='[%d/%d] Loss_D: %.4f Loss_G: %.4f = %.4f + %.4f * 1e-3' % (epoch, n_epoch, d_loss, g_loss, image_loss, adversarial_loss))
		
		if use_tensorboard:
			log_value('Discriminator Loss', cache['d_loss']/len(train_loader), epoch)
		
			log_value('Generator MSE Loss', cache['image_loss']/len(train_loader), epoch)
			log_value('Generator ADV Loss', cache['adversarial_loss']/len(train_loader), epoch)
			log_value('Generator Loss', cache['g_loss']/len(train_loader), epoch)
